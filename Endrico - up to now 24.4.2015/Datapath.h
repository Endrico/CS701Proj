...
SC_MODULE(datapath)
{
  ...
  SC_CTOR(datapath)
  {
    ...
    RF_sel->in5(SIP);
    RF_sel->in6(ER_0);
    RF_sel->in7(DM_out); // Might have to change this
	RF_sel->out(wr_data);
    ...
    my_Data_mem = new data_mem("my_Data_mem","cs701.data_mem");
    ...
    my_Prog_mem = new prog_mem("my_Prog_mem","cs701.prog_mem"); 
    ...
    SVOP->Output(SVOPi);
	 ...
	 SC_METHOD(datatransform);
		sensitive << CLK.pos() << RST << mux_DM_Data_out << data_rdadd << data_wradd << data_write << DM_q << ER_Bool << PC_out << PM_q << I_code ;
  }
};
