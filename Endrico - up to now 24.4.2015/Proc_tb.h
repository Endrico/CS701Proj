...
SC_MODULE(assignment_test_bench)
{
	...
	sc_signal<sc_uint<16> > t_dpcr_ccd;
	sc_signal<sc_uint<16> > t_ccd;
	sc_signal<sc_uint<16> > t_pcd;
	sc_signal<sc_uint<16> > t_sop;
	sc_signal<sc_uint<16> > t_svop;
	sc_signal<sc_uint<16> > t_data;
	sc_signal<sc_uint<12> > t_addr;
		...
		: my_processor("my_processor") // what is this statement doing DOG!
    {
		...
		my_processor.SIPi(t_sip);				// 16-bit	
		...
		my_processor.SOPi(t_sop);				// 16-bit
		my_processor.SVOPi(t_svop);				// 16-bit
